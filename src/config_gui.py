...
class ConfigurationGUI:
    ...
    def _create_buttons_section(self, parent: ttk.Widget):
        ...
        load_btn = ttk.Button(
            button_frame, text="Load from File", command=self._load_from_file
        )
        load_btn.pack(side="left", padx=(0, 10))

        # Save to File button
        save_btn = ttk.Button(
            button_frame, text="Save to File", command=self._save_to_file
        )
        ...
        action_frame.pack(anchor="center", pady=(15, 0))

        # Save and Start button
        save_start_btn = ttk.Button(
            action_frame,
            text="Save & Start Application",
            command=self._save_and_start,
            style="Accent.TButton",
        )
        save_start_btn.pack(side="left", padx=(0, 10))

        # Cancel button
        cancel_btn = ttk.Button(action_frame, text="Cancel", command=self._cancel)
    ...
    def _load_from_file(self):
        ...
        if file_path:
            try:
                config = PushToTalkConfig.load_from_file(file_path)
    ...
    def show_modal(self) -> str:
        ...
        self.create_gui()
        self.root.protocol("WM_DELETE_WINDOW", self._cancel)  # Handle window close
        self.root.mainloop()
...
def show_configuration_gui(
    ...
    Returns:
        Tuple of (result, config) where result is "start" or "cancel"
    """
    if config is None:
        config = PushToTalkConfig()
