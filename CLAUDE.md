...
- **Transcription System:**
  - `Transcriber` (`src/transcription.py`): OpenAI Whisper API integration
  - `LocalWhisperTranscriber` (`src/local_whisper_transcriber.py`): Local whisper.cpp integration (via pywhispercpp) with GPU acceleration
  - `LocalWhisperManager` (`src/local_whisper_manager.py`): Model download, GPU detection, and cache management
...
**Services:**
- `HotkeyService` (`src/hotkey_service.py`): Global hotkey detection using pynput library with push-to-talk and toggle modes
- `LocalWhisperManager` (`src/local_whisper_manager.py`): Model download, GPU detection, and local Whisper management
- Audio feedback utilities (`src/utils.py`): Non-blocking start/stop sounds using playsound3 and dynamic debug logging management
...
2. **Hotkey Service Thread**: Global hotkey detection (pynput library)
...
- `insertion_method`, `enable_logging`, `enable_audio_feedback`, `debug_mode`
...
The application includes a comprehensive debug mode that provides enhanced debugging capabilities:

**Dual Functionality:**
1. **Audio File Persistence**: Saves processed audio files to the current directory for debugging audio processing pipeline
2. **Enhanced Logging**: Dynamically enables DEBUG level logging to capture detailed execution information

**Implementation:**
- `debug_mode` configuration flag in `PushToTalkConfig` controls both features
- GUI checkbox labeled "Debug Mode (saves processed audio files and enables DEBUG logging)" toggles both functions
- `set_debug_logging(enabled: bool)` function in `src/utils.py` manages dynamic logging level changes
- Real-time logging level updates when debug mode is toggled during application runtime

**Technical Details:**
- Logging level changes are applied immediately without requiring application restart
- Audio file saving occurs during the audio processing pipeline when debug mode is enabled
- Debug mode is classified as a non-critical configuration field, allowing runtime updates without component reinitialization
- Enhanced debugging output includes detailed execution context and performance metrics

**Development Usage:**
- Enable debug mode to troubleshoot audio processing issues
- Review saved audio files to verify processing pipeline effectiveness
- Analyze DEBUG logs for detailed execution traces and performance bottlenecks
- Toggle debug mode on/off during development without stopping the application
...
4. **Transcribe audio using selected method:**
   - **OpenAI API**: Send processed audio to OpenAI Whisper API
   - **Local Whisper**: Process locally using whisper.cpp (via pywhispercpp) with GPU/CPU optimization
...
The application supports local speech-to-text transcription using whisper.cpp (via pywhispercpp):

**Key Features:**
- **On-demand model downloads**: Models are downloaded only when selected by the user
- **GPU acceleration**: Automatic detection and optimization for CUDA-compatible GPUs via lightweight nvidia-smi detection
- **Multiple model sizes**: Support for 12 model variants: tiny.en, tiny, base.en, base, small.en, small, medium.en, medium, large-v1, large-v2, large-v3, large-v3-turbo
- **Smart device selection**: Automatic CPU/GPU selection with manual override options
- **Compute type optimization**: Automatic selection of float16 (GPU) or int8 (CPU) for best performance

**Architecture:**
- `LocalWhisperTranscriber`: Core transcription engine with GPU detection and model loading
- `LocalWhisperManager`: Model download, status tracking, and system capability detection
- `TranscriberFactory`: Unified interface for creating OpenAI API or local transcribers
- Enhanced GUI with model selection, download progress, and system status indicators

**Enhanced User Experience:**
- **Pre-start model checks**: Automatic detection of missing models before application startup
- **Smart download confirmation**: Modal dialog with options to download, switch to API, or cancel
- **Progress tracking**: Real-time download progress with automatic dialog closure (3 seconds)
- **Automatic application startup**: Seamless transition from download completion to application start
- **Lightweight GPU detection**: Zero-dependency CUDA detection using subprocess nvidia-smi calls

**Performance Benefits:**
- No internet dependency for transcription (only refinement uses API)
- Faster processing on GPU-equipped systems (automatic float16 optimization)
- No API usage costs for transcription
- Privacy-focused local processing
- Reduced memory footprint with lightweight GPU detection (~2GB savings by removing PyTorch dependency)
- Cached GPU detection for improved startup performance
...
- Enhanced with correlation IDs, performance metrics, and comprehensive error context
- Optimized log levels to reduce verbosity while maintaining essential debugging information
- **Dynamic Debug Logging**: The `debug_mode` configuration enables DEBUG level logging via `set_debug_logging()` function in `src/utils.py`
